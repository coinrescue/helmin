...
class TxViewDelegate2 : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate2() : QAbstractItemDelegate(), unit(BitcoinUnits::HLMN)
    {
    }
...
void Coinmix::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("HLMN")
    //updateDisplayUnit();
}
...
void Coinmix::updateDarksendProgress()
{
    ...
    QString strAnonymizeHLMNAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeHLMNAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeHLMNAmount = strAnonymizeHLMNAmount.remove(strAnonymizeHLMNAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeHLMNAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeHLMNAmount * COIN) nMaxToAnonymize = nAnonymizeHLMNAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeHLMNAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeHLMNAmount));
        strAnonymizeHLMNAmount = strAnonymizeHLMNAmount.remove(strAnonymizeHLMNAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeHLMNAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
