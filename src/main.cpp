...
#if defined(NDEBUG)
#error "Helmin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Helmin Signed Message:\n";
...
int64_t GetBlockValue(int nHeight)
{
	...
	if (nHeight == 1) return 80000 * COIN;
		
	int64_t nSubsidy;
	
	if( nHeight > 1 && nHeight <= 6800 ) {
	        nSubsidy = 1 * COIN;
	} else if( nHeight > 6800 && nHeight <= 26000 ) {
	        nSubsidy = 15 * COIN;
	} else {
		nSubsidy = 17 * COIN;
	}
