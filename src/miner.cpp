...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("HLMNMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("HLMNMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("HLMNMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("helmin-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("HLMNMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running HLMNMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadHLMNMiner() exception");
    } catch (...) {
        LogPrintf("ThreadHLMNMiner() exception");
    }

    LogPrintf("ThreadHLMNMiner exiting\n");
}
